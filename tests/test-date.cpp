// test-date.cpp
//
// Runs the official structured-field-tests date.json conformance cases
// (all 17) through parse_item()/serialize_item().
...
#include "generated/date-cases.inc"
...
int main() {
    for (const auto& tc : kDateTestCases()) {
        run_case(tc);
    }
    std::cout << (g_checks - g_failures) << "/" << g_checks
              << " checks passed (date.json conformance)\n";
