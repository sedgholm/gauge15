// test-param-dict.cpp
//
// Runs the official structured-field-tests param-dict.json conformance
// cases (all 14) through parse_dictionary()/serialize_dictionary() --
// these specifically exercise Parameters attached to Dictionary members.
...
#include "generated/param-dict-cases.inc"
...
int main() {
    for (const auto& tc : kParamDictTestCases()) {
        run_case(tc);
    }
    std::cout << (g_checks - g_failures) << "/" << g_checks
              << " checks passed (param-dict.json conformance)\n";
