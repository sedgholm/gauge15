// test-binary.cpp
//
// Runs the official structured-field-tests binary.json conformance cases
// (all 15) through parse_item()/serialize_item().
...
#include "generated/binary-cases.inc"
...
int main() {
    for (const auto& tc : kBinaryTestCases()) {
        run_case(tc);
    }
    std::cout << (g_checks - g_failures) << "/" << g_checks
              << " checks passed (binary.json conformance)\n";
