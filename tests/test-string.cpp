// test-string.cpp
//
// Runs the official structured-field-tests string.json conformance cases
// (all 14) through parse_item()/serialize_item(). Same structure as
...
#include "generated/string-cases.inc"
...
int main() {
    for (const auto& tc : kStringTestCases()) {
        run_case(tc);
    }
    std::cout << (g_checks - g_failures) << "/" << g_checks
              << " checks passed (string.json conformance)\n";
