// test-display-string.cpp
//
// Runs the official structured-field-tests display-string.json
// conformance cases (all 22) through parse_item()/serialize_item().
...
#include "generated/display-string-cases.inc"
...
int main() {
    for (const auto& tc : kDisplayStringTestCases()) {
        run_case(tc);
    }
    std::cout << (g_checks - g_failures) << "/" << g_checks
              << " checks passed (display-string.json conformance)\n";
