// test-list.cpp
//
// Runs the official structured-field-tests list.json conformance cases
// (all 11) through parse_list()/serialize_list().
...
#include "generated/list-cases.inc"
...
int main() {
    for (const auto& tc : kListTestCases()) {
        run_case(tc);
    }
    std::cout << (g_checks - g_failures) << "/" << g_checks
              << " checks passed (list.json conformance)\n";
